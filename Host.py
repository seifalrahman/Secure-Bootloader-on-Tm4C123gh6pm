...
def Calculate_CRC32(Buffer, Buffer_Length):
    ...
    crc32_jamcrc = int("0b"+"1"*32, 2) - zlib.crc32(buffer_byte)
    return crc32_jamcrc
...
def Decode_CBL_Command(Command):
    ...
    if(Command == 1):
        ...
        OpenBinFile()
        '''Hash Calculations'''
        hash_512 = hashlib.sha512(BinFile.read()).digest()
        hash_512 = bytearray(hash_512) 
        BinFile.seek(0)
        ...
        while(BinFileRemainingBytes):
            ...
            hash_address = BaseMemoryAddress + BinFileReadLength
            sleep(0.1)
        
        if(hash_address % 4 !=0):
            hash_address*=4
        hash_address+=4*4
        CBL_MEM_WRITE_CMD_Len = 64+11
        BL_Host_Buffer[0]=CBL_MEM_WRITE_CMD_Len-1
        BL_Host_Buffer[1]=CBL_MEM_WRITE_CMD
        ''' Update the Host packet with the base address '''
        ...
        for byte in range(64):
            BL_Host_Buffer[7 + byte] = int(hash_512[byte])
    
        ''' Update the Host packet with the calculated CRC32 '''
        CRC32_Value = Calculate_CRC32(BL_Host_Buffer, CBL_MEM_WRITE_CMD_Len - 4) 
        CRC32_Value = CRC32_Value & 0xFFFFFFFF
        BL_Host_Buffer[7 + 64] = Word_Value_To_Byte_Value(CRC32_Value, 1, 1)
        BL_Host_Buffer[8 + 64] = Word_Value_To_Byte_Value(CRC32_Value, 2, 1)
        BL_Host_Buffer[9 + 64] = Word_Value_To_Byte_Value(CRC32_Value, 3, 1)
        BL_Host_Buffer[10+ 64] = Word_Value_To_Byte_Value(CRC32_Value, 4, 1)
        ''' Send the packet length to the bootloader '''
        Write_Data_To_Serial_Port(BL_Host_Buffer[0], 1)
